...
export default function WorkoutTimerPage() {
  ...
  // 讀取設定，若沒有則使用預設值
  const settings =
    JSON.parse(localStorage.getItem(`workout_${exerciseId}`)) || {};
  ...
  const sendPhaseNotification = (currentPhase, endTimestamp) => {
    if ("Notification" in window && Notification.permission === "granted") {
      const timeStr = new Date(endTimestamp).toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
        second: "2-digit",
      });
  ...
  const resetCurrentPhase = () => {
    ...
    const duration =
      phase === "prepare" ? 3 : phase === "work" ? workTime : restTime;
  ...
  const strokeDashoffset =
    circumference - (timeLeft / totalPhaseTime) * circumference;
    ...
    >
      ...
      <div className="controls-group">
        <div className="main-controls">
          ...
          {/* 跳過按鈕 >> */}
          <button onClick={handleNextPhase} className="icon-btn" title="Skip">
            ⏭️
          </button>
        </div>
